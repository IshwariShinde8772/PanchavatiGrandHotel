...
export default function MyBookings() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow={t("nav.myTrips")} title={t("customer.tripsTitle")} description={t("customer.tripsDescription")} />
          ...
          <div key={booking.id} className="section-card overflow-hidden md:grid md:grid-cols-[220px_minmax(0,1fr)]">
            ...
            <div className="p-5">
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                {booking.payment_status === "pending" ? <Button as={Link} to="/customer/transactions" variant="secondary">{t("customer.payNow")}</Button> : null}
                {booking.status === "checked_out" ? <Button variant="gold">{t("customer.downloadBill")}</Button> : null}
                {["confirmed", "pending"].includes(booking.status) ? <Button variant="secondary" onClick={() => setSelectedId(booking.id)}>{t("common.cancel")}</Button> : null}
              </div>
            </div>
          </div>
        ))}
