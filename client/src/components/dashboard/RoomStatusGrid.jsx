...
export default function RoomStatusGrid({ rooms = [] }) {
  return (
    <div className="section-card p-5">
      <h3 className="font-heading text-2xl">Room Status Grid</h3>
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
        {rooms.map((room) => (
          ...
          >
            ...
            <p className="text-sm font-medium opacity-90">{room.category}</p>
            <p className="mt-3 text-[10px] uppercase font-bold tracking-[0.15em] opacity-80">{room.status}</p>
          </div>
        ))}
      </div>
    </div>
